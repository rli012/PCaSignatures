...
#########################################################################
###      Evaluation of Survial Analysis Models (Training - Test)      ###
#########################################################################
...
if (training.set=='GSE116918_Metastasis') {
  phenoData$time_to_bcr <- phenoData$time_to_metastasis
  phenoData$bcr_status <- phenoData$metastasis_status
}
...
for (test.set in test.sets) {
  ...
  if (test.set=='GSE116918_Metastasis') {
    phenoData$time_to_bcr <- phenoData$time_to_metastasis
    phenoData$bcr_status <- phenoData$metastasis_status
  }
  ...
  for (signature.name in signatures) {
    ...
    genes <- Reduce(intersect, list(signature.genes, colnames(training.geno.all), colnames(test.geno.all)))
    #genes <- Reduce(intersect, list(genes, colnames(training.geno.all), colnames(test.geno.all)))
    ...
    write.csv(res, file=paste0('report/Survival/TrainTestScale/Signature/Survival_',
                               model, '_', signature.name, '_', training.set, '_', test.set, '_TrainTest_Signature.csv'),
              quote = F)

    ##### Smples used for classification
    # ### ========================================= ###
    # write.csv(res, file=paste0('report/SurvivalC/TrainTestScale/Signature/Survival_',
    #                            model, '_', signature.name, '_', training.set, '_', test.set, '_TrainTest_Signature.csv'),
    #           quote = F)
    # ### ========================================= ###
