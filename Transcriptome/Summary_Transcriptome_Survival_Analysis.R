...
### Final Models
models <- c('CoxLasso','CoxRidge','SuperPC','plsRcox','RandomForest')

signature.name <- 'Transcriptome'
...
for (dataset in datasets) {
  ...
  for (model in models) {
    ...
    fl.name <- paste0('report/Survival/CV10Scale/Transcriptome/Survival_',
...
saveRDS(dataForBoxPlot, file='report/Summary/Summary_Survival_Transcriptome_Intra_Dataset.RDS')
...
for (training.set in datasets) {
  ...
  for (test.set in test.sets) {
    ...
    for (model in models) {
      ...
      fl.name <- paste0('report/Survival/TrainTestScale/Transcriptome/Survival_',
...
saveRDS(dataForBoxPlot, file='report/Summary/Summary_Survival_Transcriptome_Inter_Dataset.RDS')
...
res <- read.csv(file=paste0('report/Survival/CV10Scale/Transcriptome/Survival_', model, '_', signature.name, '_', dataset, '.csv'),
...
for (dataset in datasets) {
  ...
  for (model in models) {
    for (signature.name in signatures) {
      ...
      fl <- paste0('report/Survival/CV10Scale/Transcriptome/Survival_', model, '_', signature.name, '_', dataset, '.csv')
  ...
  for (test.set in test.sets) {
    ...
    for (model in models) {
      for (signature.name in signatures) {
        ...
        fl <- paste0('report/Survival/TrainTestScale/Transcriptome/Survival_', 
                     model, '_', signature.name, '_', training.set, '_', test.set, '_TrainTest_Signature.csv')
