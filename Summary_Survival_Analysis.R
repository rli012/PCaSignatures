  ...
  for (model in models) {
    ...
    for (signature.name in signatures) {
      ...
      if (! file.exists(fl.name)) {
        ...
        stats <- c(stats, c(dataset, model, signature.name, rep(NA,9)))
        statsList[[i]] <- stats
        next
      }
      ...
      c <- concordance.index(x=res$risk.score, 
                             surv.time=res$bcr.time, 
                             surv.event=res$bcr.status, 
                             #cl=res$risk.group,
                             method="noether")
      c.index <- c$c.index
  ...
  for (test.set in test.sets) {
    ...
    for (model in models) {
      ...
      for (signature.name in signatures) {
        ...
        if (! file.exists(fl.name)) {
          print (paste('NA:', fl.name))
          stats <- c(stats, c(training.set, test.set, model, signature.name, rep(NA,9)))
          statsList[[i]] <- stats
          next
        }
...
colnames(statsDF) <- c('Training', 'Test', 'Model', 'Signature', 'C', 'Cox.HR','Cox.Lower95',
                       'Cox.Upper96','Cox.P','KM.HR','KM.Lower95','KM.Upper95','KM.P')

statsDF[,5:13] <- apply(statsDF[,5:13], 2, as.numeric)
